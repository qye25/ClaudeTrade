...
VALID_PROVIDERS = {
    "google", "openai", "anthropic", "groq", "cerebras", "openrouter", "mistral",
    "deepseek", "xai", "qwen", "qwen-cn", "glm", "glm-cn", "minimax", "minimax-cn",
    "kimi", "nvidia", "ollama", "openai_compatible",
}
...
def _extract_json_object(text: str) -> dict:
    ...
    if not cleaned:
        raise ValueError("Research planner returned an empty response")
    ...
    try:
        ...
        for match in re.finditer(r"\{", cleaned):
            try:
                ...
                continue
        raise ValueError("Research planner did not return a valid JSON object")
...
def choose_research_symbols(graph: TradingAgentsGraph, portfolio: Portfolio, limit: int = 3) -> tuple[tuple[str, ...], str]:
...
concentration, leverage, risk, catalysts, or unusual exposure. Do not invent symbols.
...
""".strip()
    ...
    if not selected:
        selected = [p.symbol for p in sorted(portfolio.positions, key=lambda p: p.market_value, reverse=True)[:limit]]
...
def resolve_run_config(mode: str) -> tuple[tuple[str, ...], dict]:
    ...
    if env_provider not in VALID_PROVIDERS:
        raise ValueError(f"Invalid provider '{env_provider}'. Valid choices: {sorted(VALID_PROVIDERS)}")
    ...
    config["deep_think_llm"] = os.getenv("TRADINGAGENTS_DEEP_THINK_LLM", config.get("deep_think_llm", "gemini-3.1-flash-lite"))
    config["quick_think_llm"] = os.getenv("TRADINGAGENTS_QUICK_THINK_LLM", config.get("quick_think_llm", "gemini-3.1-flash-lite"))
...
async def _open_robinhood_session():
    ...
    async with AsyncClient(auth=oauth_provider, follow_redirects=True) as http_client:
        async with streamable_http_client(ROBINHOOD_MCP_URL, http_client=http_client, terminate_on_close=False) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                if missing:
                    raise RuntimeError(f"Robinhood MCP is missing required read-only tools: {sorted(missing)}")
...
def _compact_report(text: str, max_chars: int = 12000) -> str:
    text = (text or "").strip()
...
def run_sequential_research(graph: TradingAgentsGraph, research_symbols: tuple[str, ...], portfolio_context: str, analysis_date: str) -> tuple[dict[str, dict[str, str]], float]:
    ...
    for symbol in research_symbols:
        ...
        state = graph.propagator.create_initial_state(symbol, analysis_date, asset_type="stock", instrument_context=instrument_context, portfolio_context=portfolio_context, research_symbols=(symbol,))
        state["minimal_mode"] = True
        final_state = research_workflow.invoke(state, **graph.propagator.get_graph_args())
        reports[symbol] = {
...
def run_portfolio_manager(graph: TradingAgentsGraph, anchor: str, portfolio_context: str, reports: dict[str, dict[str, str]], analysis_date: str) -> tuple[str, float]:
    ...
    state = graph.propagator.create_initial_state(anchor, analysis_date, asset_type="stock", instrument_context=instrument_context, portfolio_context=portfolio_context, research_symbols=tuple(reports))
    market_parts, news_parts, plan_parts = [], [], []
    ...
    return str(final_state.get("final_trade_decision", "")).strip(), time.perf_counter() - started
    ...
    async with _open_robinhood_session() as (session, account_number):
        ...
        if symbol:
            ...
            if anchor not in known_symbols:
                raise ValueError(f"Anchor symbol {anchor} is not in the live portfolio. Available: {sorted(known_symbols)}")
        else:
        ...
        try:
            ...
            research_limit = 3
        research_symbols, planner_reason = choose_research_symbols(graph, portfolio, limit=research_limit)
        planner_elapsed = time.perf_counter() - planner_start
        if anchor not in research_symbols:
            research_symbols = (anchor,) + tuple(value for value in research_symbols if value != anchor)[: max(0, research_limit - 1)]
        ...
        if mode.lower() == "fast":
            reports, research_elapsed = run_sequential_research(graph, research_symbols, context, analysis_date)
            decision, pm_elapsed = run_portfolio_manager(graph, anchor, context, reports, analysis_date)
        ...
        print(f"Research planner:  {planner_elapsed:.2f}s")
        if mode.lower() == "fast":
            print(f"Sequential research:{research_elapsed:.2f}s")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run TradingAgents against the live Robinhood portfolio in read-only mode.")
    parser.add_argument("symbol", nargs="?", help="Optional research anchor symbol from the current portfolio.")
    parser.add_argument("--mode", choices=("fast", "full"), default=os.getenv("TRADINGAGENTS_MODE", "fast"), help="Research mode. Fast defaults to market+news and minimal thinking; full uses all four analysts.")
    parser.add_argument("--inspect-tax-lot", metavar="SYMBOL", help="Print raw Robinhood tax-lot fields instead of running TradingAgents.")
