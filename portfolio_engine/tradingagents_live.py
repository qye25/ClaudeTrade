    ...
    for position in sorted(portfolio.positions, key=lambda p: p.market_value, reverse=True):
        ...
        if position.tax_lots:
            for lot in position.tax_lots:
                lines.append(
                    "  lot: " f"qty={lot.quantity}, cost_basis=${lot.cost_basis}, acquired={lot.acquisition_date}, "
...
Choose up to {limit} existing portfolio holdings that deserve detailed market/news research before the Portfolio Manager makes ONE decision for the entire account.
Prioritize concentration, leverage, risk, catalysts, or unusual exposure. Return ONLY JSON: {{"symbols":["UPRO","TSLA"],"reason":"one concise sentence"}}
AVAILABLE HOLDINGS:
...
def resolve_run_config(mode: str) -> tuple[tuple[str, ...], dict]:
    ...
    if env_provider == "groq" and requested_mode == "full":
        config["quick_think_llm"] = os.getenv("TRADINGAGENTS_QUICK_THINK_LLM", "llama-3.3-70b-versatile")
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("fast", "full"), default=os.getenv("TRADINGAGENTS_MODE", "fast"), help="Research mode.")
